...
jq --arg description "$description" --arg pipeline_url "$PIPELINE_URL" --arg '.description = $description +" The pipeline url is: "+ $pipeline_url' < body-parsed.json > INPUT.tmp && mv INPUT.tmp body-parsed.json
if [ -n "${alias}" ];then
